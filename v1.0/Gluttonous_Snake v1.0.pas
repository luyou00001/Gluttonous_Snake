...
uses crt;
var length,i,j,choice,score,speed,direction,temp,acceleration,head,item_num,enlarge,max,appear:integer;
    die,move:Boolean;
    item: array[1..5] of integer;
    snake: array[1..760] of integer;
    key:char;

procedure Aboutgame;
begin
          gotoxy(21,3);write('             About the game             ');
          gotoxy(21,7);write('  Gluttonous Snake v1.0 by Sen Sen      ');
          gotoxy(21,8);write('                                        ');
         ...
         if item[i]=-1 then
            begin
                 ...
                 1:
                   begin
                        enlarge:=enlarge+1;
                        score:=score+(max*20*(1000+speed) div 2000);
                        item[i]:=0;
                   end;
                 2:
                   begin
                 ...
                 5:
                   begin
                        score:=score+(max*100*(1000+speed) div 2000);
                        item[i]:=0;
                   end;
                 end;
             end;
...
begin
     ...
     if item[1]=0 then
        begin
             repeat
                   item[1]:=random(760)+1;
             until (snake[item[1]]=0) and (item[1]<>item[2]) and (item[1]<>item[3]) and (item[1]<>item[4]) and (item[1]<>item[5]);
        ...
        begin
             ...
             if item_num<6 then
                begin
                     ...
                     until (snake[item[item_num]]=0) and (item[item_num]<>item[1]);
...
begin
     ...
     1:
       begin
            if (head <= 40) or (snake[head-40]>0) then die:=True
            else
                begin
                     head:=head-40;
                     Check_Pick;
                     ...
                     if move then
                        begin
                             for i:= 1 to 760 do if snake[i]=length then snake[i-40]:=length+1;
     ...
     2:
       begin
            if (head >= 721) or (snake[head+40]>0) then die:=True
            else
                begin
                     head:=head+40;
                     Check_Pick;
     ...
     3:
       begin
            if (head mod 40 = 1) or (snake[head-1]>0) then die:=True
            else
                begin
                     head:=head-1;
                     Check_Pick;
                     ...
                     if move then
                        begin
                             for i:= 1 to 760 do if snake[i]=length then snake[i-1]:=length+1;
                             for i:= 1 to 760 do if snake[i]=1 then snake[i]:=-1;
                             for i:= 1 to 760 do if snake[i]>0 then snake[i]:=snake[i]-1;
                        end;
                end;
     ...
     4:
       begin
            if (head mod 40 = 0) or (snake[head+1]>0) then die:=True
            else
                begin
                     head:=head+1;
                     Check_Pick;
...
begin
     ...
     gotoxy(21,13);write('   XXX  X   X X   X XXXXX               ');
     gotoxy(21,14);write('                                        ');
     gotoxy(21,15);write('   XXX  X   X XXXXX XXXX                ');
     gotoxy(21,16);write('  X   X X   X X     X   X  Max Length:  ');
...
begin
     ...
     textcolor(15);
     gotoxy(21,15);write('       Press right arrow to start       ');
     repeat
           key:=readkey;
     until key=#77;
     direction:=4;
...
begin
     ...
     repeat
           ...
           write('X                  X                                        X                  X');            {2}
           write('X    *********>    X   Gluttonous Snake  v1.0  by Sen Sen   X  Pickable Items  X');            {3}
           write('X    *             X                                        X ---------------- X');            {4}
